...
function run(F){
 ...
 test('Gastar menos que el plan es favorable',()=>{const b=F.monthlyBudgets({month:'2026-01',budgets:[{category:'Casa',planned:300}],transactions:[{date:'2026-01-02',kind:'expense',category:'Casa',amount:250}]});const row=b.categories[0];near(row.deviation,50);near(row.remaining,50);if(!row.favorable||row.status!=='under')throw Error('El ahorro frente al plan debe ser favorable');});
